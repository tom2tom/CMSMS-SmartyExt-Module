(function(global, $, jQuery, Cookies, help_functions) {
  var OE = global.OE; //TODO disentangle from 111 theme
  ...
  jQuery(function() {
    ...
    $('#page_tabs > div').on('click', function() {
      Cookies.set('smarty_ext_tab_active', this.id);
    });

    /* copy to clipboard stuff */
    $('.click-to-copy').on('click', function(evt) {
      evt.preventDefault();
  ...
  function _set_active_tab(tab_name) {
    $('#page_tabs > div').removeClass('active');
  ...
  help_functions.help_error_message = function(message_text) {
    ...
    $('#helper_error').detach();
    $('#oe_mainarea').prepend(output); //TODO disentangle from 111 theme
    $('#helper_error').hide();
   ...
   * the user clicked and dragged, or just clicked:
  ...
  help_functions.copyToClipboard = function(element, dragging) {
    ...
    document.execCommand('copy'); //dodgy!
    $temp.remove();
