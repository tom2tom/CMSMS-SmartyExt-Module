...
final class smx
{
  ...
  static public function get_parent_alias($alias = '')
  {
    ...
    return $node->getParent()->get_tag('alias');
  }
  ...
  public static function is_child_of($test_parent, $test_child)
  {
    ...
    if(!$test_parent) { return FALSE; }
    
    if(!$test_child) { return FALSE; }
  ...
  public static function get_root_alias($alias = '')
  {
    ...
    if( 0 === count($stack) ) { return ''; }
  
    return $stack[count($stack) - 1]->get_tag('alias');
  }
  ...
  static public function get_children($alias = '', $showall = FALSE)
  {
    ...
    if(!is_array($child_nodes) || 0 === count($child_nodes) ) { return []; }
    ...
    foreach($child_nodes as $node)
    {
      ...
      $row       = [
        'alias'    => $content->Alias(), 'id' => $content->Id(), 'title' => $content->Name(),
   ...
   * @return string
   */
  public static function get_page_content($alias = NULL, $block = NULL)
  {
    $content = NULL;
    $block   = trim($block);
    ...
    {
      return $content->GetPropertyValue($block);
    }
   ...
   */
  public static function get_sibling($dir, $alias)
  {
    $contentops = \ContentOperations::get_instance();
    ...
    // @todo: could use the tree here (find the node, get it's parent, the parent's children are the sibling)
    $content = $contentops->LoadContentFromAlias($alias);
    ...
    return \CmsApp::get_instance()->GetDb()->GetOne(sprintf($query, $thechar, $order), [$content->ParentId(), $content->ItemOrder()]);
  }
  ...
  /**
   * Get a parallel content object given a different root alias
   ...
   */
  public static function get_parallel_content($new_root, $current_page = NULL)
  {
    ...
    return $contentops->LoadContentFromAlias($new_pageid);
  }
   ...
   */
  public static function get_parallel_page($new_root, $current_page = NULL)
  {
