<?php
# Smarty plugin: page_protect - Single Page Simple Password Protection
# Version 1.2.0
# (c) 2012 -2014 Fernando Morgado (JoMorg) jomorg.morg@gmail.com
#
# Plugin allows protecting a page with a password
# Perhaps this belongs with the MAMS module ?
#
...
function smarty_function_content_protect($params, $template)
{
    // initialize a few vars
    $current_page_alias = cms_utils::get_current_alias();
    $logout_alias = (!empty($params['logout_alias'])) ? $params['logout_alias'] : $current_page_alias;
    $login_alias = (!empty($params['login_alias'])) ? $params['login_alias'] : null;
    $assign_var = (!empty($params['assign'])) ? $params['assign'] : 'cp_logged_in';

    // some of the params have to persist through all the page request,
    ...
    if (!isset($cookie_name)) {
        $cookie_name = (!empty($params['cookie_name'])) ? $params['cookie_name'] : 'cp_auth';
    ...
    if (!isset($error_msg)) {
        $error_msg = (!empty($params['error_msg'])) ? $params['error_msg'] : 'The password is not correct.';
    ...
    if (!isset($welcome_msg)) {
        $welcome_msg = (!empty($params['welcome_msg'])) ? $params['welcome_msg'] : 'Please enter the password to access this page.';
    ...
    switch ($action) {
        case 'form':
            ...
            if (!$allow) {
                $login_btn = (!empty($params['login_btn'])) ? $params['login_btn'] : 'Login'; //TODO lang
                ...
                $logout_btn = (!empty($params['logout_btn'])) ? $params['logout_btn'] : 'Logout'; //TODO lang
            ...
            if (isset($params['assign_output'])) {
                $template->assign($params['assign_output'], $html);
            } else {
                $ret = $html;
            }
            break;
    ...
    $template->assign($assign_var, $allow);

    return (isset($ret)) ? $ret : ''; // @since 1.2 -> if we come from forms action we may have $ret set
}
